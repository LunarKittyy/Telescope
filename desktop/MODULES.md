...
- `split_zoom(zoom, pan_x, pan_y, caps)` - pure: the user's framing as the phone's share (`PhoneZoom(ratio, crop, x, y)`, sent as the `zoom` control) plus the desktop's remainder `(zoom, pan_x, pan_y)`; together they always frame the same. The phone's ratio is the largest centred zoom the window still fits in (so a multi-lens camera picks its telephoto when it can), then a freeform crop cuts the window itself and a centre-only one goes as far as the window still fits. `PhoneZoomCaps.from_camera(cam)` reads `zoomRatioMax`, `cropZoomMax`, `freeformCrop` from the current `/v1/state` camera; `None` (a lens that can't zoom, an older phone app, not streaming) leaves it all to the desktop, as before.
- The plugin tracks the current camera from `on_phone_state` and `bus.camera_switched`, and re-splits on any zoom/pan/caps change; the phone only hears about changes (and again on each stream start).
