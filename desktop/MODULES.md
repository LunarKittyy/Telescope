...
Registration order: `SetupPlugin → ConnectionPlugin → CameraControlPlugin → StreamOutputPlugin → TransformsPlugin → PreviewPlugin → OnboardingPlugin → MonitoringPlugin → UpdatesPlugin → StartupPlugin → WaitScreenPlugin`. Preview comes before Onboarding so the stage is listening when the checklist first says whether it needs the space.
...
- Implements the public **`HostServices`** contract (see `plugin.py`): `schedule_save()`, `save_now()`, `switch_device()`, `forget_device_settings()`, `reconnect_stream()`, `send_notification()`, `is_streaming()`, `is_starting()` (a Start is waking the phone, before the worker exists), `stop_stream()`, `update_stream_output()`, `restart_vcam_canvas()`, `quit_app()` (really quits, tray or not), `start_stream(interactive=True)` (False: no password prompt, problems go to a banner), `set_keep_in_tray(keep)` (close hides to the tray even when idle), `show_issue(key, Issue)` / `clear_issue(key=None)` (problem banners above the body; every banner clears when a stream connects), `plugin_config(name)` / `apply_preset(name, cfg)` (another plugin's config, and handing it one to apply; used by Presets). Plugins only call these; private internals (`_worker`, `_stop()`) are hidden.
...
- **Getting a dropped stream back.** The worker's `"reconnecting"` status starts `_begin_recovery()`: `bus.stream_lost`, then every `_RECOVER_RETRY_MS` (3 s) a background `conn.recovery_probe()` job resolves the route again. A phone that answers while streaming gets the stream moved there once per route (`conn.adopt_stream_route()` for the URL and a fresh USB forward, a new `PhoneControlClient`, `worker.retarget(url)`); one that answers but isn't streaming ends the stream with a "The phone stopped streaming" banner. `_recovery_gen` drops probes from before the stream came back or stopped. So a pulled cable falls back to Wi-Fi, and a replugged one gets a working forward again.
...
- `HostServices` (typing.Protocol): the public surface a plugin may call on its `host` handle - `schedule_save`, `save_now`, `switch_device`, `forget_device_settings`, `reconnect_stream`, `send_notification`, `is_streaming`, `is_starting`, `stop_stream`, `update_stream_output`, `restart_vcam_canvas`, `quit_app`, `start_stream(interactive)`, `set_keep_in_tray`, `show_issue`, `clear_issue`, `plugin_config`, `apply_preset`. Structural typing only (`TelescopeWindow` implements it without inheriting). Keeps plugins off private window internals.
...
- `TelescopePlugin`: override `setup`, `create_panel`, `on_stream_starting` (just before the worker opens the virtual camera), `on_stream_start`, `on_stream_stop`, `on_phone_state`, `process_frame`, `get_config`, `set_config`, `apply_preset`, `shutdown`.
...
- `EventBus(QObject)`: signals - `stream_started`, `stream_stopped`, `stream_connected`, `stream_lost` (frames stopped; the host is looking for a route back), `phone_state_updated`, `device_changed`, `phones_changed(int)`, `add_phone_requested`, `setup_needed(bool)`, `camera_switched`, `resolution_change_requested`, `update_requested` (the phone app is newer; opens the update dialog), `focus_point_picked(u, v)` (preview click, in the frame as shown) → Transforms → `focus_point(x, y)` (in the phone's frame) → Camera control sends `focus_point`, `focus_point_available(bool)` (Camera control → Preview's crosshair), `phone_ready(phone_id, ready)` (from Connection's idle status checks only, never from Start's own resolve), `max_zoom_changed(int)` (Setup's Max zoom → Transforms' slider range), `view_dragged(du, dv)` / `view_scrolled(factor, u, v)` (Preview's mouse → Transforms' pan and zoom), `view_pannable(bool)` and `lens_boxes(boxes, moved)` (Transforms → Preview's hand cursor and lens outlines), `vcam_opened(w, h)` (the stream opened the virtual camera at this size), `camera_watched(bool)` (Wait screen: an app started or stopped reading the camera). `phones_changed`, `add_phone_requested` and `setup_needed` are how the checklist, Connection and Preview cooperate without calling each other.

### `stream.py`
**StreamWorker(QThread)** - video capture and virtual camera output.
- Reads the authenticated stream with `MjpegReader` (`mjpeg_reader.py`) or, for a URL ending `.h264`, `H264Reader` (`h264_reader.py`); both send the bearer token and report `last_frame_bytes` for the throughput readout. Writes to the virtual camera opened by `vcam.open_camera()` and emits `vcam_opened(w, h)` once it's open.
...
- Auto-reconnects on stream drop (`RECONNECT_DELAY = 3s`). `retarget(url)` switches the URL it reconnects to and skips the rest of the wait; the host calls it when the phone is reachable another way.

### `vcam.py`
The virtual camera outside a stream. `open_camera(w, h, fps, fmt)` opens `/dev/video11` (Linux) or the UnityCapture device named `UC_NAME`, falling back to any free one (Windows); `stream.py` uses it too.
- `WaitScreen` holds the camera while nothing streams and shows `load_frames(path, w, h)` on it: the chosen image or GIF fitted to the camera size, or the default screen. Frames are prepared once (I420 on Linux, so a send is a plain write), sent once a second while nobody reads the camera, and at `STILL_PERIOD` or the GIF's own timing while someone does. A GIF keeps every nth frame past `FRAME_BUDGET` (64 MB).
- `CameraWatch(on_change)` reports whether an app is reading the camera: the v4l2loopback client-usage event (0.13+), a `/proc` scan every 5 s on older drivers (`camera_holders()`), or on Windows the `UnityCapture_Want` event an app's filter sets for each frame it wants (`_uc_want_event_name()` mirrors pyvirtualcam's numbering).
...
### `audio.py`
The phone's mic into the virtual mic. `JitterBuffer` (pure): silence until `TARGET_MS` (60) has built up, silence-padding on a gap, and dropping the oldest audio back to the target past `MAX_MS` (150), which absorbs clock drift. `AudioWorker`: a reader thread (`/v1/audio` with the bearer token; on a refusal it reports the phone's `error` and retries every 3 s, so allowing the mic on the phone just starts it) and a writer thread that drains 10 ms at a time into a sink at the sink's pace. Sinks: `FifoSink` (Linux: writes the virtual mic's FIFO, paced to real time itself since the pipe source reads whatever arrives, with the pipe shrunk to one page so a stalled reader can't build up delay) and `SoundDeviceSink` (Windows). `on_status(kind, text)` comes from the worker threads.

### `platform/virtual_mic.py`
Linux: `linux_setup()` loads `module-pipe-source` (`telescope_mic`, "Telescope Microphone", s16le 48 kHz mono) on `fifo_path()` with `pactl`. It's an input only, so nothing shows up as a speaker. It first unloads any module an earlier run left (including the null sink + remap the first builds used), and retries without the description if a server rejects it. `linux_teardown(ids)`. Windows: `find_vb_cable(devices)` finds CABLE Input in `sounddevice.query_devices()`. Runners are injectable for tests.
...
### `diagnostics.py`
The log and Copy diagnostics. `install(app_dir)` (from `main.py`) opens `events`, an `EventLog` handler, on `log_dir()/telescope.log` (`/tmp/telescope-<user>` or `%TEMP%\Telescope`), links `telescope.log` in the app folder to it where symlinks work, adds it to the root logger and hooks sys and thread exceptions. It takes Telescope's info and up, other libraries' warnings and up, crashes and `note()`s (the host notes each status line and banner). A repeat of the latest line becomes a "repeated N more times" line. The file rotates at `MAX_BYTES` into `telescope.log.1`; without a file it keeps `REPORT_LINES` in memory. `sanitize()` strips URLs, IPs, tokens and the home path before a line is stored. `report(state, qt_platform)` builds the text: version, OS, desktop session, then each plugin's `diagnostics()` lines, then the last `REPORT_LINES` log lines (across a restart). The host's `diagnostics_report()` is what the Advanced dialog's button copies.
...
Cross-platform constants and helpers: `IS_LINUX`, `IS_WINDOWS`, `adb_available()`, `adb_device_states()` (serial + state, including `unauthorized`), `adb_devices()`, `adb_forward(port)`, `adb_forward_auto(remote_port, serial)` (adb picks the local port), `adb_unforward(port)`, `adb_reverse(port)`, `adb_unreverse(port)`, `adb_broadcast_pair(payload)`, `adb_install(serial, apk, timeout)` → `(ok, detail)` with plain-language failures (different signing key, downgrade), `adb_exe()`, `bundled_apk_path()`, `_run(cmd)`. `_run()` returns an error tuple instead of raising when adb isn't installed, and on Windows runs with `NO_WINDOW` (`CREATE_NO_WINDOW`) so the packaged app doesn't flash a console for every adb call. On Windows, `ensure_adb_server()` (called before each adb command) starts `adb nodaemon server` as Telescope's own child, unless a server is already listening on 5037, and `platform/winjob.py` puts it in a job object with KILL_ON_JOB_CLOSE so it dies with Telescope even on a crash. `stop_adb_server()` runs at quit and before an update installs, since a running adb.exe locks `platform-tools`.
...
`update_menu_entry(save_icon)` keeps `$XDG_DATA_HOME/applications/telescope.desktop` and its 256px icon (drawn by `create_app_icon`) pointing at this copy; `main.py` calls it on every Linux start except from a git checkout (`is_dev_checkout()`), and it only writes when the entry changed. `APP_ID` is also the Qt desktop file name, so docks match the window to the entry.
...
- Card rows: **Phone** (status), **Using** (live route), **Connect via** (Automatic / USB only / Wi-Fi only), a note with the fix when something's wrong or why a plugged-in cable isn't used, and **Switch to USB** when the phone is plugged in mid-stream over Wi-Fi. The card header's action is **Add phone**. While streaming, the status reads Connecting… until `bus.stream_connected`, and Reconnecting… from `bus.stream_lost` until it's back.
- `recovery_probe()` → a blocking job returning the current `Resolution`; `adopt_stream_route(route)` → moves the stream's route (and its USB forward, always opened fresh since adb drops forwards with the cable) and returns the new video URL.
...
- UI: flip (H/V segmented), rotation (None / 90 CW / 180 / 90 CCW), zoom slider (1× up to Setup's Max zoom, 10× by default, through `bus.max_zoom_changed`), pan left/right and up/down (enabled only when zoomed), and a Reset action in the card header that drives the widgets so the handlers do the rest.
- `split_zoom(zoom, pan_x, pan_y, caps)` - pure: the user's framing as the phone's share (`PhoneZoom(ratio, crop, x, y)`, sent as the `zoom` control) plus the desktop's remainder `(zoom, pan_x, pan_y)`; together they always frame the same. The phone's ratio only steps between lenses (`lensZooms`, the ratios where a multi-lens camera's longer lenses take over): the longest lens whose centred view still holds the window, with a 5% margin to move onto a lens (capped at the zoom itself, so a centred window always gets its lens) and none to stay on it (`current_ratio`). Phones animate ratio changes, so a ratio that followed the pan made the picture breathe. A freeform crop then cuts the window itself out of that lens's view; with a centre-only crop the phone stops at the lens and this computer does the rest, so the phone's share never changes with the pan. `PhoneZoomCaps.from_camera(cam)` reads `zoomRatioMax`, `cropZoomMax`, `freeformCrop`, `lensZooms` from the current `/v1/state` camera; `None` (a lens that can't zoom, an older phone app, not streaming) leaves it all to the desktop, as before.
- The plugin tracks the current camera from `on_phone_state` and `bus.camera_switched`, and re-splits on any zoom/pan/caps change; the phone only hears about changes (and again on each stream start). The zoom slider's tooltip says where the zoom happens. `lens_note(zoom, sent_ratio, caps, live, default, tele, settled)` - pure: `(level, text)` for the dot on the zoom value (a child of the value label, outside the layout, so it never moves anything); level `""` hides it. `"on"` (lavender) says "Switched to …" when `/v1/state`'s `active_lens` (the physical lens a multi-lens camera streams from) differs from the one live at ratio 1. `"off"` (red, wins) explains the fallback when the pan put the ratio below the lens a centred window would get, or, once the last ratio sent had `_LENS_SETTLE_S` to take, that the phone stayed on its default lens by itself. The zoom slider snaps to each lens at `lens_step(lens)` (rounded up, so the mark always gets the lens; `_lens_ratio` allows float slack, as 1/(1/z) can land a hair under z) and resets to 1x on a double-click. A preview drag sticks at the centre per axis within `_PAN_SNAP` of the frame, from a raw pan (`_raw_pan`) that keeps counting underneath, so dragging on lets go without a jump; any other framing change drops it. The plugin remembers the default and telephoto names from `on_phone_state` and forgets them when the caps change.
- `shown_to_view` / `view_to_shown` - pure, continuous and unclamped: a point in the frame as shown to and from the full view of the main lens, by the zoom/pan asked for (not the desktop share). `pan_for(centre, zoom)` inverts `_window_centre`. Dragging the preview moves the window by the drag mapped through them, so the picture follows the mouse in any rotation/flip; scrolling zooms by the factor, stops on a lens mark it crosses (`zoom_detent`), and re-solves the pan so the point under the mouse stays put (a finer `_wheel_zoom` lets trackpad steps add up). Pan is kept as floats and saved as such, finer than the pan sliders. `lens_boxes(caps, ...)` - each longer lens's centred 1/ratio of the view, as shown.
- `inverse_map(u, v, w, h, zoom, pan_x, pan_y, flip_h, flip_v, rotation)` - pure: a point in the transformed frame back to the phone's frame (undoes rotate, flip, then zoom). The plugin maps `bus.focus_point_picked` with it through the desktop's remainder only, using the last frame's size, and emits `bus.focus_point`; the phone maps it on through its own crop.
- `process_frame(frame)` - applies the desktop's share of the zoom crop then flip/rotate; runs on the worker thread. Reads plain Python attrs (`flip_h`, `flip_v`, `rotation`, and `_desktop_crop` as one tuple) written by the Qt thread; GIL makes these reads atomic.
...
### `plugins/wait_screen.py`
**WaitScreenPlugin** - runs a `vcam.WaitScreen` from app start to quit, except from `on_stream_starting` to `on_stream_stop`, at the Setup canvas size or else the size the last stream opened the camera at (`last_size`, from `bus.vcam_opened`), so a call that's already reading keeps working when the stream takes over (v4l2loopback readers lose the picture on a size change). Runs `vcam.CameraWatch` and re-emits it as `bus.camera_watched`. **Wait screen…** in the settings menu opens `WaitScreenDialog` (preview, Choose image…, Use default); a chosen image is copied next to the config as `wait_screen.<ext>`. Config keys: `image`, `last_size` (global).

### `plugins/startup.py`
**StartupPlugin** - three checkable settings-menu entries. **Start streaming when the phone is ready** listens to `bus.phone_ready` and calls `host.start_stream(interactive=False)` once per arrival: a Stop or an attempt holds it until the phone reports not ready (or another phone is selected). It also keeps the window in the tray on close. **Open Telescope when I sign in** calls `platform/autostart.py`. **Stream only while an app is using the camera** listens to `bus.camera_watched` and starts the same way once the phone is ready; a stream it started stops `WATCH_STOP_DELAY_MS` (15 s) after the last reader lets go. A Stop holds it until the reader lets go. Config keys: `auto_stream`, `watch_stream` (global).
...
- `AdvancedDialog`: v4l2loopback status, load/unload and load at boot (Linux), UnityCapture status and reinstall (or **Rename**, to Telescope, for an older registration) plus adb status (Windows), installing an APK over USB, the virtual camera canvas (presets or custom, applied through `host.restart_vcam_canvas()`), and Max zoom (2-30×, applied straight away through `bus.max_zoom_changed`).
- `get_canvas_dims()` → `(w, h)` or `(None, None)` for auto; read by `app.py` at stream start.
- Config keys: `canvas_preset`, `custom_canvas_w`, `custom_canvas_h`, `max_zoom` (default 10; anything else out of range falls back to it). `set_config` emits `max_zoom_changed`, and global configs load before per-phone ones, so the slider's range is in place before a saved zoom is set.
