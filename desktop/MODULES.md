...
### `diagnostics.py`
Copy diagnostics. `install()` (from `main.py`) adds `events`, a `RecentEvents` log handler, to the root logger, plus sys and thread excepthooks. It keeps the last 50 warnings, errors, crashes and `note()`s (the host notes each status line and banner) in memory only; a repeat of the latest one bumps a count. `sanitize()` strips URLs, IPs, tokens and the home path. `report(state, qt_platform)` builds the text: version, OS, desktop session, then each plugin's `diagnostics()` lines, then the recent events. The host's `diagnostics_report()` is what the Advanced dialog's button copies.
