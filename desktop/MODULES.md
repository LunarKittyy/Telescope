...
- The plugin tracks the current camera from `on_phone_state` and `bus.camera_switched`, and re-splits on any zoom/pan/caps change; the phone only hears about changes (and again on each stream start). The zoom slider's tooltip says where the zoom happens. `lens_note(zoom, sent_ratio, caps, live, default, tele)` - pure: the hover text for a red dot on the zoom value (a child of the value label, outside the layout, so it never moves anything), or `""` to hide it. It says "Switched to …" when `/v1/state`'s `active_lens` (the physical lens a multi-lens camera streams from) differs from the one live at ratio 1, and explains the fallback when the pan put the ratio below the lens a centred window would get. The plugin remembers the default and telephoto names from `on_phone_state` and forgets them when the caps change.
