...
- `ZoomSlider` - a `NoScrollSlider` that paints a dot on the groove at each value in `set_marks()` (under the handle, lighter on the filled part); `mark_x(value)` is the handle's centre there, asked of the style.
